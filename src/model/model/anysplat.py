...
class AnySplat(nn.Module, huggingface_hub.PyTorchModelHubMixin):
    ...
    def inference(self,
        ...
        #  Pass global prune percentage to the encoder at inference.
        global_prune_percent = 10
        print(f"global_prune_percent: {global_prune_percent}")
        encoder_output = self.encoder(context_image, global_prune_percent, global_step=0, visualization_dump=None)
    ...
    def forward(self, 
        ...
        #  Randomized global prune percentage during training forward.
        global_prune_percent = (9.99 * np.random.rand() ) * 10
        encoder_output = self.encoder(context_image, global_prune_percent, global_step, visualization_dump=visualization_dump)
        gaussians, pred_context_pose = encoder_output.gaussians, encoder_output.pred_context_pose
