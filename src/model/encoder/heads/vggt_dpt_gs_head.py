...
class VGGT_DPT_GS_Head(DPTHead):
    def __init__(self, 
        ...
        #  Conv to fuse the prune mask channel into the head features.
        self.mask_merge = nn.Conv2d(head_features_1+1, head_features_1, kernel_size=3, stride=1, padding=1)

    #  Pass the per-pixel pruning mask to the Gaussian head.
    def forward(self, encoder_tokens: List[torch.Tensor], depths, imgs, mask, patch_start_idx: int = 5, image_size=None, conf=None, frames_chunk_size: int = 8):
        ...
        if frames_chunk_size is None or frames_chunk_size >= S:
            return self._forward_impl(encoder_tokens, imgs, mask, patch_start_idx)
        ...
        for frames_start_idx in range(0, S, frames_chunk_size):
            ...
            chunk_output = self._forward_impl(
                encoder_tokens, imgs, mask, patch_start_idx, frames_start_idx, frames_end_idx
            )
    ...
    def _forward_impl(self, encoder_tokens: List[torch.Tensor], imgs, mask, patch_start_idx: int = 5, frames_start_idx: int = None, frames_end_idx: int = None):
        
        if frames_start_idx is not None and frames_end_idx is not None:
            imgs = imgs[:, frames_start_idx:frames_end_idx]
            mask = mask[:, frames_start_idx:frames_end_idx]
        ...
        direct_img_feat = self.input_merger(imgs.flatten(0,1)) # 21,3,224,224 -> 21,128,224,224

        out = F.interpolate(out, size=(H, W), mode='bilinear', align_corners=True)
        out = out + direct_img_feat
        out = torch.cat((out, mask.flatten(0,1)), dim=1)
        out = self.mask_merge(out)
