...
def setup_args():
    ...
    parser.add_argument('--global_prune_percent', type=float, default=10, help='global_prune_percent')
    parser.add_argument('--output_path', type=str, default="outputs/nvs", help='Path to output directory')
    parser.add_argument('--ckpt', type=str, default=None,
                        help='Optional fine-tuned Lightning .ckpt. If omitted, the pretrained '
                             'lhjiang/anysplat weights are used as-is.')
...
def evaluate(args: argparse.Namespace):
    ...
    # Optionally override the pretrained weights with a fine-tuned checkpoint.
    # Lightning saves parameters under a "model." prefix, which is stripped here.
    ...
    encoder_output = model.encoder(
        ctx_images,
        args.global_prune_percent, # global_prune_percent
        global_step=0,
        visualization_dump={},
    )
